#!/bin/bash
#install_demo_configuration.sh [-y]
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
echo "## Search Guard Demo Installer ##"
echo "Warning: Do not use on production or public reachable systems"

if [ "$1" != "-y" ]; then
	read -r -p "Continue? [y/N] " response
	case "$response" in
	    [yY][eE][sS]|[yY])
	        ;;
	    *)
	        exit 0
	        ;;
	esac
fi

set -e
BASE_DIR="/usr/share/elasticsearch"
ES_CONF_FILE="$BASE_DIR/config/elasticsearch.yml"
ES_BIN_DIR="$BASE_DIR/bin"
ES_PLUGINS_DIR="$BASE_DIR/plugins"
ES_LIB_PATH="$BASE_DIR/lib"
SUDO_CMD=""
BASE_64_DECODE_CMD="base64 -d"
ES_INSTALL_TYPE=".tar.gz"


if  test -f "$ES_CONF_FILE"; then
    :
else
    echo "Unable to determine elasticsearch config directory. Quit."
    exit -1
fi

if [ ! -d $ES_BIN_DIR ]; then
	echo "Unable to determine elasticsearch bin directory. Quit."
	exit -1
fi

if [ ! -d $ES_PLUGINS_DIR ]; then
	echo "Unable to determine elasticsearch plugins directory. Quit."
	exit -1
fi

if [ ! -d $ES_LIB_PATH ]; then
	echo "Unable to determine elasticsearch lib directory. Quit."
	exit -1
fi

if [ "$ES_INSTALL_TYPE" != "rpm/deb" ];then
    ES_CONF_DIR=$(dirname "${ES_CONF_FILE}")
    ES_CONF_DIR=`cd "$ES_CONF_DIR" ; pwd`
else
    ES_CONF_DIR="/etc/elasticsearch"
fi

ES_CONF_FILE="$ES_CONF_DIR/elasticsearch.yml"

if  grep --quiet -i searchguard $ES_CONF_FILE; then
  echo "$ES_CONF_FILE seems to be already configured for Search Guard. Quit."
  exit -1
fi


if [ ! -d "$ES_PLUGINS_DIR/search-guard-5" ]; then
  echo "Search Guard plugin not installed. Quit."
  exit -1
fi

ES_VERSION=("$ES_LIB_PATH/elasticsearch-*.jar")
ES_VERSION=$(echo $ES_VERSION | sed 's/.*elasticsearch-\(.*\)\.jar/\1/')

SG_VERSION=("$ES_PLUGINS_DIR/search-guard-5/search-guard-5-*.jar")
SG_VERSION=$(echo $SG_VERSION | sed 's/.*search-guard-5-\(.*\)\.jar/\1/')

echo "Elasticsearch install type: $ES_INSTALL_TYPE"
echo "Elasticsearch config dir: $ES_CONF_DIR"
echo "Detected Elasticsearch Version: $ES_VERSION"
echo "Detected Search Guard Version: $SG_VERSION"

SG_ADMIN_KSEYSTORE_B64=""
SG_KEYSTORE_B64=""
SG_TRUSTSTORE_B64=""

if [ "$(uname)" == "Darwin" ]; then
    BASE_64_DECODE_CMD="base64 -D"
fi

echo "$SG_ADMIN_KSEYSTORE_B64" | $BASE_64_DECODE_CMD |  tee "$ES_CONF_DIR/kirk.jks" > /dev/null
echo "$SG_KEYSTORE_B64" | $BASE_64_DECODE_CMD |  tee "$ES_CONF_DIR/keystore.jks" > /dev/null
echo "$SG_TRUSTSTORE_B64" | $BASE_64_DECODE_CMD |  tee "$ES_CONF_DIR/truststore.jks" > /dev/null

echo "" |  tee -a  $ES_CONF_FILE
echo "######## Start Search Guard Demo Configuration ########" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.transport.keystore_filepath: keystore.jks" |  tee -a  $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.transport.truststore_filepath: truststore.jks" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.transport.enforce_hostname_verification: false" |  tee -a  $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.http.enabled: true" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.http.keystore_filepath: keystore.jks" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.http.truststore_filepath: truststore.jks" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.transport.enable_openssl_if_available: true" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.ssl.http.enable_openssl_if_available: true" |  tee -a $ES_CONF_FILE > /dev/null
echo "searchguard.authcz.admin_dn:" |  tee -a $ES_CONF_FILE > /dev/null
echo "  - CN=kirk,OU=client,O=client,L=test, C=de" |  tee -a $ES_CONF_FILE > /dev/null
echo "" |  tee -a $ES_CONF_FILE > /dev/null
echo "cluster.name: searchguard_demo" |  tee -a $ES_CONF_FILE > /dev/null
echo "network.host: 0.0.0.0" |  tee -a $ES_CONF_FILE > /dev/null
echo "######## End Search Guard Demo Configuration ########" |  tee -a $ES_CONF_FILE > /dev/null

 chmod +x "$ES_PLUGINS_DIR/search-guard-5/tools/sgadmin.sh"

ES_PLUGINS_DIR=`cd "$ES_PLUGINS_DIR" ; pwd`

echo "### Success"
echo "### Execute this script now on all your nodes and then start all nodes"
echo "### After the whole cluster is up execute: "
echo "#!/bin/bash" |  tee sgadmin_demo.sh > /dev/null
echo  "$ES_PLUGINS_DIR/search-guard-5/tools/sgadmin.sh" -cd "$ES_PLUGINS_DIR/search-guard-5/sgconfig" -cn searchguard_demo -ks "$ES_CONF_DIR/kirk.jks" -ts "$ES_CONF_DIR/truststore.jks" -nhnv |  tee -a sgadmin_demo.sh > /dev/null
 chmod +x sgadmin_demo.sh
 cat sgadmin_demo.sh | tail -1
echo "### or run ./sgadmin_demo.sh"
echo "### Then open https://localhost:9200 an login with admin/admin"
echo "### (Just ignore the ssl certificate warning because we installed a self signed demo certificate)"